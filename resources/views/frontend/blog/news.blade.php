				...
				<article>
					<div class="art-header">
						<div class="entry-title"> 
							<h2>{{ $singl_post -> title }}</h2>
						</div>
						<div class="info">By @foreach (json_decode($singl_post -> tags) as $tag) <a href="{{ $tag -> slug }}">{{ $tag -> tname }}</a>@endforeach on {{ date('Y, d, F', strtotime($singl_post -> created_at)) }}</div>
					</div>
					<div class="art-content">
						<img src="{{ url('storage/post_feature/', $singl_post -> feature) }}" />
						<div class="excerpt"><p>{!! htmlspecialchars_decode($singl_post -> content) !!}</p>
						<blockquote><p>{!! htmlspecialchars_decode($singl_post -> quote) !!}</p></blockquote>
						<h2>Heading 1</h2>
						<p>{!! htmlspecialchars_decode($singl_post -> head1) !!}</p>
						<h2>Heading 2</h2>
						<p>{!! htmlspecialchars_decode($singl_post -> head2) !!}</p>
						<h2>Heading 3</h2>
						...
						<p><code>Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet.</code>
							{!! htmlspecialchars_decode($singl_post -> head3) !!}
						</p>
						<div class="note">
						  <ol>
							<li>Lorem ipsum</li>
